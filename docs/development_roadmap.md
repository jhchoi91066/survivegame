...
#### 🎮 게임 메커니즘 대대적 개편 (턴제 → 퍼즐 게임)
- [x] **핵심 시스템 재설계**
  - [x] 턴제 이동 시스템 제거
  - [x] 장애물 제거 중심 퍼즐 시스템으로 전환
  - [x] 게임 플로우 변경: 계획 단계 → 실행 단계 → 결과

- [x] **장애물 타입 확장**
  - [x] 새로운 장애물 타입 추가:
    - [x] 🔥 불 - 의사+물(자원2) OR 자연소멸(시간5초), 확산 가능
    - [x] 🌲 나무 - 엔지니어(자원1) OR 불로 태우기
    - [x] 💧 물댐 - 엔지니어(자원3) OR 폭탄(자원1, 범람 위험)
    - [x] 💣 폭탄/폭약 - 반경 1칸 파괴
    - [x] 🧊 얼음 - 불로 녹이기 OR 자연소멸(시간10초)
    - [x] 🌫️ 안개 지역 - 아이로 정찰 필요
  - [x] 장애물별 제거 방법 정의 (자원, 생존자 역할, 연쇄 효과)

- [x] **장애물 의존성 시스템**
  - [x] blockedBy 시스템: A 제거 전까지 B 제거 불가
  - [x] blocksIds 시스템: A 제거 시 B 잠금 해제
  - [ ] 의존성 시각화 (잠긴 장애물 표시)

- [x] **연쇄 반응 시스템**
  - [x] 물 범람: 물댐 폭파 시 주변 침수
  - [x] 불 소화: 물이 불을 끔
  - [x] 폭발: 주변 장애물 파괴
  - [x] 불 확산: 불이 인접 나무로 번짐
  - [x] 얼음 녹이기: 불이 얼음을 녹임
...
- [x] **생존자 역할 강화**
  - [x] 생존자별 장애물 제거 능력 재정의
  - [x] 각 생존자 사용 제한 (1회 사용 제한)
  - [x] 생존자 시너지 시스템:
    - [x] 👷+👨‍⚕️ = 안전 폭파 (주변 피해 없음)
    - [x] 👨‍🍳+👶 = 비상식량 (시간 +20초)
    - [x] 👨‍⚕️+👨‍🍳 = 해독제 (독가스 영구 제거)
...
- [x] **장애물 클릭 인터랙션 UI**
  - [x] 장애물 클릭 시 제거 방법 모달
  - [x] 각 방법의 비용, 위험도, 연쇄 효과 표시
  - [x] 제거 불가능한 장애물 피드백 (이유 표시)
  - [x] 잠긴 장애물 UI (먼저 제거해야 할 장애물 표시)
...
- [x] 통계 시스템
  ...
  - [x] 업적 시스템 기초 구현
- [x] 튜토리얼 개선
  - [x] 인터랙티브 튜토리얼 (실제 게임에서 가이드)
  - [x] 각 단계마다 손가락 포인터 표시
  - [x] 튜토리얼 중 특정 액션 강제 (다음으로 넘어가기 전)
- [x] 성능 최적화
